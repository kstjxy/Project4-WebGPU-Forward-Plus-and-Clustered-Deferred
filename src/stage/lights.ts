...
export class Lights {
    ...
    constructor(camera: Camera) {
        ...
        const entriesPerCluster = 1 + shaders.constants.maxLightsPerCluster; // count + indices
        const clusterBufferSizeBytes = numClusters * entriesPerCluster * 4; // u32
        this.clusterLightsStorageBuffer = device.createBuffer({
            label: "cluster lights buffer",
            size: clusterBufferSizeBytes,
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
        });

        this.clusteringComputeBindGroupLayout = device.createBindGroupLayout({
            label: "clustering compute BGL",
            entries: [
                { // lightSet
                    binding: 0,
                    visibility: GPUShaderStage.COMPUTE,
        ...
        this.clusteringComputeBindGroup = device.createBindGroup({
            ...
            entries: [
                ...
                { binding: 2, resource: { buffer: this.clusterLightsStorageBuffer } }
            ]
        });
    ...
    doLightClustering(encoder: GPUCommandEncoder) {
        // run the light clustering compute pass
        const pass = encoder.beginComputePass({ label: "light clustering pass" });
        pass.setPipeline(this.clusteringComputePipeline);
        pass.setBindGroup(0, this.clusteringComputeBindGroup);
        const wgSize = 4; // must match @workgroup_size in shader (per axis)
        const x = Math.ceil(shaders.constants.clusterCountX / wgSize);
        const y = Math.ceil(shaders.constants.clusterCountY / wgSize);
        const z = Math.ceil(shaders.constants.clusterCountZ / wgSize);
